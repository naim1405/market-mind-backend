...
This project ships with a Dockerfile for the app image and a docker-compose.yml for local orchestration.

### Services and profiles

- `mongodb` runs MongoDB 7 with a single-node replica set (required for Prisma transactions).
- `db-init-replica` initializes the replica set on first run.
- `server` (profile `app`) builds and runs the Node.js API container.

By default, `docker compose up` starts only MongoDB and the replica init helper. To include the API server, enable the `app` profile.

### Common commands

Start only MongoDB (default profile):
...
- The API container runs `npx prisma migrate deploy` on startup, then starts the server.
- Logs are persisted to a named volume (`app_log`).
- The MongoDB port is published on host `27018` (container `27017`).
